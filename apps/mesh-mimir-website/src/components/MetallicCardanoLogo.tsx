...
export function MetallicCardanoLogo({
  ...
  useEffect(() => {
    async function loadCardanoLogo() {
      try {
        setIsLoading(true);
        ...
        // Use the parseLogoImage function to process it for the shader
        const result = (await parseLogoImage(file)) as {
          imageData: ImageData;
          pngBlob: Blob;
        };
        ...
        console.error("Error loading Cardano logo:", error);
        setIsLoading(false);
      }
    }
  ...
  if (isLoading) {
    return (
      <div
        className={`flex items-center justify-center bg-surface/20 rounded-2xl border border-primary/20 ${className}`}
